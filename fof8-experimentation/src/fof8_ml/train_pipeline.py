...
warnings.filterwarnings("ignore", category=FutureWarning, module="hydra_plugins.hydra_optuna_sweeper")
...
from sklearn.metrics import confusion_matrix, f1_score, mean_squared_error, mean_absolute_error, recall_score, precision_score, precision_recall_curve, auc, roc_auc_score
...
from fof8_ml.data.dataset import build_economic_dataset, apply_position_mask
...
def get_model_wrapper(model_name: str, stage: str, random_seed: int, params: dict, use_gpu: bool = False, thread_count: int = -1):
    ...
    if stage == "stage1":
        if "catboost" in model_name:
            return CatBoostClassifierWrapper(random_seed=random_seed, use_gpu=use_gpu, thread_count=thread_count, **params)
        ...
        if "catboost" in model_name:
            return CatBoostRegressorWrapper(random_seed=random_seed, use_gpu=use_gpu, thread_count=thread_count, **params)
        elif "xgb" in model_name:
...
def main(cfg: DictConfig):
        ...
        if existing_runs:
            ...
            try:
                with mlflow.start_run(run_name=sweep_name) as sweep_parent:
                    ...
                    if "hydra" in cfg and "sweeper" in cfg.hydra and "search_space" in cfg.hydra.sweeper:
                        search_space = OmegaConf.to_container(cfg.hydra.sweeper.search_space, resolve=True)
    ...
    valid_end_year = final_sim_year - cfg.data.right_censor_buffer
    total_valid_years = valid_end_year - valid_start_year + 1
    
    # 3. Calculate Chronological Split
    test_years_count = int(total_valid_years * cfg.data.test_split_pct)
    ...
    if not (is_sweep and cfg.get("quiet_sweep", False)):
        ...
        print(f"Training Set: {train_year_range} ({train_year_range[1] - train_year_range[0] + 1} draft classes)")
        print(f"Holdout Set: {test_year_range} ({test_year_range[1] - test_year_range[0] + 1} draft classes)")

    # 4. Build/Load datasets
    cache_dir = os.path.abspath(os.path.join(exp_root, cfg.data.cache_dir))
    
    if not (is_sweep and cfg.get("quiet_sweep", False)):
        print("\nChecking Training Dataset Cache...")
    X_train, y_train_df, meta_train = load_cached_dataset(cache_dir, cfg, "train", train_year_range)
    if X_train is None:
        if not (is_sweep and cfg.get("quiet_sweep", False)):
            print("Cache miss. Building Training Dataset from raw data...")
        X_train, y_train_df, meta_train = build_economic_dataset(
            absolute_raw_path, cfg.data.league_name, train_year_range, final_sim_year,
            positions=cfg.data.positions, active_team_id=cfg.data.get("active_team_id"),
            merit_threshold=cfg.target.merit_threshold
        )
        save_cached_dataset(cache_dir, cfg, X_train, y_train_df, meta_train, "train", train_year_range)

    if not (is_sweep and cfg.get("quiet_sweep", False)):
        print("\nChecking Holdout Dataset Cache...")
    X_test, y_test_df, meta_test = load_cached_dataset(cache_dir, cfg, "test", test_year_range)
    if X_test is None:
        if not (is_sweep and cfg.get("quiet_sweep", False)):
            print("Cache miss. Building Holdout Testing Dataset from raw data...")
        X_test, y_test_df, meta_test = build_economic_dataset(
            absolute_raw_path, cfg.data.league_name, test_year_range, final_sim_year,
            positions=cfg.data.positions, active_team_id=cfg.data.get("active_team_id"),
            merit_threshold=cfg.target.merit_threshold
        )
        save_cached_dataset(cache_dir, cfg, X_test, y_test_df, meta_test, "test", test_year_range)
    
    if cfg.data.get("mask_positional_features", False):
        if not (is_sweep and cfg.get("quiet_sweep", False)):
            print("Applying In-Memory Positional Feature Masking...")
        X_train = apply_position_mask(X_train)
        X_test = apply_position_mask(X_test)

    include_features = cfg.data.get("include_features")
    if include_features:
        ...
        print(f"Applying Feature Ablation: Keeping {len(include_cols)} features matching {include_features}")
        X_train = X_train.select(include_cols)
        X_test = X_test.select(include_cols)
        
    exclude_features = cfg.data.get("exclude_features")
    if exclude_features:
        ...
        if cols_to_drop:
            print(f"Applying Feature Ablation: Excluding {len(cols_to_drop)} features matching {exclude_features}")
    ...
    with mlflow.start_run(
        ...
        if not (is_sweep and cfg.get("quiet_sweep", False)):
            print(f"Stage 1 Class Balance - Positive Merit (Hits): {n_pos}, Negative Merit (Busts): {n_neg}, Ratio: {pos_weight:.2f}")
        ...
        if not stage1_run_id:
            with mlflow.start_run(run_name="Stage1_Sieve_Classifier", nested=True) as stage1_run:
                ...
                skf = StratifiedKFold(n_splits=cfg.cv.n_folds, shuffle=True, random_state=cfg.data.seed)
                ...
                for fold, (train_idx, val_idx) in enumerate(skf.split(indices, y_cls)):
                    ...
                    model = get_model_wrapper(cfg.stage1_model.name, "stage1", cfg.data.seed, params, use_gpu=use_gpu, thread_count=thread_count)
                ...
                stage1_model = get_model_wrapper(cfg.stage1_model.name, "stage1", cfg.data.seed, final_params_s1, use_gpu=cfg.get("use_gpu", False), thread_count=cfg.stage1_model.params.get("thread_count", -1))
                ...
                if cfg.diagnostics.log_importance or cfg.diagnostics.log_shap:
                    ...
                    log_feature_importance(stage1_model.model, X_train.columns, "Stage 1 Importance", is_cb, X=X_train.to_pandas(), log_shap=cfg.diagnostics.log_shap)
        ...
        if cfg.get("train_stage2", True):
            with mlflow.start_run(run_name="Stage2_Intensity_Regressor", nested=True) as stage2_run:
                ...
                kf = KFold(n_splits=cfg.cv.n_folds, shuffle=True, random_state=cfg.data.seed)
                ...
                for fold, (train_idx, val_idx) in enumerate(kf.split(indices_reg)):
                    ...
                    model = get_model_wrapper(cfg.stage2_model.name, "stage2", cfg.data.seed, params, use_gpu=use_gpu, thread_count=thread_count)
                ...
                s2_oof_rmse = np.sqrt(mean_squared_error(np.expm1(y_reg_target), np.expm1(oof_preds_reg)))
                ...
                stage2_model = get_model_wrapper(cfg.stage2_model.name, "stage2", cfg.data.seed, final_params_s2, use_gpu=cfg.get("use_gpu", False), thread_count=cfg.stage2_model.params.get("thread_count", -1))
                stage2_model.fit(X_reg, y_reg_target)
                stage2_model.log_model("stage2_model")

                if cfg.diagnostics.log_importance or cfg.diagnostics.log_shap:
                    is_cb = "catboost" in cfg.stage2_model.name.lower()
                    log_feature_importance(stage2_model.model, X_reg.columns, "Stage 2 Importance", is_cb, X=X_reg.to_pandas(), log_shap=cfg.diagnostics.log_shap)
        ...
        opt_metric = cfg.get("optimization", {}).get("metric", "s2_oof_rmse" if cfg.get("train_stage2", True) else "s1_oof_pr_auc")
        higher_is_better = cfg.get("optimization", {}).get("direction", "maximize") == "maximize"
        
        current_score = available_metrics.get(opt_metric)
        if current_score is None:
            raise ValueError(f"Metric '{opt_metric}' is not available for optimization. Available metrics: {list(available_metrics.keys())}")
        ...
        if sweep_run_id:
            ...
            print("-" * 60)
            print(f"LATEST TRIAL RESULT: {current_score:.4f} ({score_label}) ({'IMPROVEMENT' if is_new_best else 'No improvement'})")
