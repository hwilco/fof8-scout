...
def main(cfg: DictConfig):
    ...
    runs = client.search_runs(experiment_ids=[experiment.experiment_id], filter_string="tags.mlflow.runName LIKE 'Pipeline_%'")
    ...
    stage1_run = next((r for r in nested_runs if r.data.tags.get("mlflow.runName") == "Stage1_Sieve_Classifier"), None)
    stage2_run = next((r for r in nested_runs if r.data.tags.get("mlflow.runName") == "Stage2_Intensity_Regressor"), None)
    ...
    # Fetch Age
    with pl.StringCache():
        df_info = loader.scan_file("player_information.csv", year=2144).select(["Player_ID", "Draft_Year", "Year_Born"]).collect()
        
    df_features = df_features.join(df_info, on="Player_ID", how="left").with_columns(
        (pl.col("Draft_Year") - pl.col("Year_Born")).alias("Age")
    ...
    df_results = df_features.select(["Player_ID", "First_Name", "Last_Name", "Position_Group"]).with_columns([
        pl.Series("P_Sieve", p_sieve),
        pl.Series("Expected_DGO", expected_dgo)
    ])
    ...
    df_results = df_results.with_columns(
        (pl.col("Universal_EDV").rank(descending=False).over("Position_Group") / 
         pl.len().over("Position_Group")).alias("Positional_Tp")
    )
