...
The Stage 1 Sieve will see the terrible positional Z-scores and output a P_Sieve (survival probability) of something like 0.001. Even if the Stage 2 Intensity model gets confused by the OOD weight and spits out an artificially high Expected DGO, the final Universal_EDV calculation (0.001 * Expected_DGO) will crush their value down to zero, naturally burying them at the bottom of the draft board for that position.
