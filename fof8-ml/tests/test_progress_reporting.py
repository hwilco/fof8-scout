...
def test_classifier_skips_final_refit_by_default_in_validation_holdout_mode(monkeypatch):
...
def test_classifier_uses_runtime_progress_setting_for_opt_in_final_refit(monkeypatch):
    captured: dict[str, int] = {}
    ...
    cfg = OmegaConf.create(
        {
            ...
            "seed": 42,
            "use_gpu": False,
            "runtime": {"catboost_progress_every": 17, "refit_final_model": True},
        }
    )
    data = SimpleNamespace(
        X_train=pl.DataFrame({"feature": [1.0, 2.0]}),
...
def test_regressor_skips_final_refit_by_default_in_validation_holdout_mode(monkeypatch):
    captured: dict[str, int] = {}
...
def test_regressor_uses_runtime_progress_setting_for_opt_in_final_refit(monkeypatch):
    captured: dict[str, int] = {}
    ...
    cfg = OmegaConf.create(
        {
            ...
            "seed": 42,
            "use_gpu": False,
            "runtime": {"catboost_progress_every": 23, "refit_final_model": True},
        }
    )
    data = SimpleNamespace(
        X_train=pl.DataFrame({"feature": [1.0, 2.0]}),
