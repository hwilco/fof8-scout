...
def strategy_25th_percentile(lf_av: pl.LazyFrame) -> pl.LazyFrame:
    ...
    baseline = (
        lf_av
        .filter(pl.col("Season_AV") > 0)
        .group_by(["Year", "Position_Group"])
        .agg(
            pl.col("Season_AV").quantile(0.25).alias("Replacement_AV")
        )
    )

    return (
        lf_av
        .join(baseline, on=["Year", "Position_Group"], how="left")
        .with_columns(
            pl.col("Replacement_AV").fill_null(0.0)
        )
        .with_columns(
            (pl.col("Season_AV") - pl.col("Replacement_AV")).alias("Season_VORP")
        )
    )
...
def strategy_n_plus_one(lf_av: pl.LazyFrame) -> pl.LazyFrame:
    ...
    starters_per_position_group_per_team = [
        ("QB", 1), ("RB", 1), ("FB", 1), ("WR", 3), ("TE", 1), ("T", 2), ("G", 2), ("C", 1), 
        ("DE", 2), ("DT", 2), ("OLB", 2), ("ILB", 1), ("CB", 2), ("S", 2), ("K", 1), ("P", 1), ("LS", 1)
    ]
    starters_per_position_group = [(position, NUM_TEAMS * starters_per_team) for position, starters_per_team in starters_per_position_group_per_team]
    starter_counts = (
        pl.DataFrame(
    ...
    baseline = (
        ...
        .filter(pl.col("Season_AV") > 0)
        .with_columns(
            pl.col("Season_AV").rank(method="ordinal", descending=True).over(["Year", "Position_Group"]).alias("Pos_Rank")
        )
        .with_columns(
        ...
        .with_columns(
            ...
            pl.min_horizontal(pl.col("League_Starters") + 1, pl.col("Max_Pos_Rank")).alias("Target_Rank")
        )
        .filter(pl.col("Pos_Rank") == pl.col("Target_Rank"))
        .select([
            "Year", 
            "Position_Group", 
            pl.col("Season_AV").alias("Replacement_AV")
        ])
    )
...
def strategy_hybrid_baseline(lf_av: pl.LazyFrame) -> pl.LazyFrame:
    ...
    ironman_baseline = (
        lf
        .filter(pl.col("Position_Group").is_in(list(ironman_starters.keys())))
        ...
        .with_columns(
            pl.col("Valid_Starter_Rank").max().over(["Year", "Position_Group"]).alias("Valid_Starter_Count")
        )
        .filter(
            pl.col("Valid_Starter_Rank") > (pl.col("Valid_Starter_Count") - 5)
        )
        .group_by(["Year", "Position_Group"])
    ...
    snap_baseline = (
        ...
        .with_columns(
            (pl.col("Snap_Rank") / pl.col("Total_Active")).alias("Snap_Percentile")
        )
        .filter(
            (pl.col("Snap_Percentile") > 0.05) & 
            (pl.col("Snap_Percentile") <= 0.25)
        )
        .group_by(["Year", "Position_Group"])
    ...
    return (
        lf
        .join(ironman_baseline, on=["Year", "Position_Group"], how="left")
        ...
        .drop([
            "Relevant_Snaps", "AV_Per_Snap",
            "Ironman_Rep_Rate", "Snap_Rep_Rate", "Replacement_Rate"
        ])
    )
